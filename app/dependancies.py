...
def sign_up():
    with st.form(key="signup", clear_on_submit=True):
        ...
        username = st.text_input(
            ":blue[Usuário]", placeholder="Digite seu Nome de Usuário"
        )
        password1 = st.text_input(
            ":blue[Senha]", placeholder="Digite sua Senha", type="password"
        )
        password2 = st.text_input(
            ":blue[Confirmar Senha]", placeholder="Confirme sua Senha", type="password"
        )
...
def input_dados():
    with st.form(key="dados", clear_on_submit=True):
        ...
        nome = st.text_input(
            ":blue[Nome]", placeholder="Digite seu Nome Completo", value=nome_value
        )
        doc_identidade = st.text_input(
            ":blue[Identidade]", placeholder="Digite seu documento da identidade"
        )
        org_emissor = st.text_input(
            ":blue[Órgão Emissor]", placeholder="Digite o órgão emissor"
        )
        ...
        data_nascimento = st.date_input(
            "Data de nascimento", value=None, min_value=min_date
        )
